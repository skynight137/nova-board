...
/**
 * CTRL/ALT/SHIFT are modifier toggles: tapping one arms it, and it's applied as metaState to
 * the *next* key press (e.g. CTRL then C sends Ctrl+C) then automatically clears, mirroring how
...
class HotkeyController(private val getInputConnection: () -> InputConnection?) {
    ...
    fun build(context: Context, row: LinearLayout, onModifierChanged: () -> Unit) {
        ...
        row.addView(toggleButton(context, "CTRL", { ctrlArmed }) { ctrlArmed = it; onModifierChanged() })
        row.addView(toggleButton(context, "ALT", { altArmed }) { altArmed = it; onModifierChanged() })
    ...
    /** Sends a plain character through as a key event with any armed modifiers applied, then clears them. */
    ...
    private fun toggleButton(context: Context, label: String, isOn: () -> Boolean, onToggle: (Boolean) -> Unit) =
        TextView(context).apply {
            text = label
    ...
    private fun applyHotkeyStyle(view: TextView) {
        ...
        lp.setMargins((3 * density).toInt(), (4 * density).toInt(), (3 * density).toInt(), (4 * density).toInt())
