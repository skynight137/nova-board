...
/**
 * A compact, on-device suggestion engine. It's intentionally simple (frequency dictionary + bigram
 * next-word map + edit-distance autocorrect) so it needs no network calls or model download; swap
 * `wordFrequency`/`bigrams` for a larger corpus, or replace this class with an ML Kit / on-device
 * LLM-backed implementation later without touching the IME service.
 */
class SuggestionEngine {

    // word -> usage frequency (higher = more common). Seed list; extend freely.
    private val seedWordFrequency =
        mapOf(
            "the" to 100,
            "be" to 95,
            "to" to 94,
    ...
    private val wordFrequency = seedWordFrequency.toMutableMap()
    ...
    /** Discard learned frequency changes while retaining the built-in dictionary. */
    fun clearLearnedData() {
        wordFrequency.clear()
        wordFrequency.putAll(seedWordFrequency)
    }

    /**
     * Returns up to 3 suggestions for the given typing state. If [currentWord] is non-empty:
     * autocorrect/completion candidates. If [currentWord] is empty: next-word prediction based on
     * [previousWord].
     */
