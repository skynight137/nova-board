...
enum class KeyType {
    CHAR, SHIFT, BACKSPACE, ENTER, SPACE, SYMBOLS, LETTERS, SWITCH_NUMBER_ROW, EMOJI, PERIOD, COMMA
}

/**
 * A single key on the keyboard.
 *
 * [label] is what's drawn on the key. [popupChars] are the characters offered on long-press
 * (e.g. long-pressing "e" offers "è é ê ë 3"), matching the "long press symbol" requirement.
 * [flexWeight] lets keys size proportionally within a row (e.g. space bar is much wider).
 */
data class Key(
    val type: KeyType,
    val label: String = "",
...
object KeyboardLayouts {
    ...
    private val numberRow = KeyRow(
        "1234567890".map { Key(KeyType.CHAR, it.toString(), it.code) }
    )
    ...
    val letters = KeyboardPage(listOf(numberRow, qwertyRow1, qwertyRow2, qwertyRow3, bottomRow))

    private val symbolsRow1 = KeyRow("@#$_&-+()".map { Key(KeyType.CHAR, it.toString(), it.code) })
    private val symbolsRow2 = KeyRow("*\"':;!?".map { Key(KeyType.CHAR, it.toString(), it.code) })
    private val symbolsRow3 = KeyRow(
        listOf(Key(KeyType.LETTERS, "=\\<", flexWeight = 1.5f)) +
            "/[]{}%".map { Key(KeyType.CHAR, it.toString(), it.code) } +
            listOf(Key(KeyType.BACKSPACE, "\u232b", flexWeight = 1.5f))
    )

    /** Symbols page shown when the "123" key is tapped. */
    val symbols = KeyboardPage(listOf(numberRow, symbolsRow1, symbolsRow2, symbolsRow3, bottomRow))
}
